...
if($_POST["subcategorygroups"]){
	
	$subcategories_id = $_POST["subcategories"];
	$subcategorygroups = $_POST["subcategorygroups"];
	
	$query_rs_products = sprintf("SELECT * FROM v_products_list WHERE categories_id = %s AND subcategories_subcategorygroup_id=$subcategorygroups AND subcategories_id=$subcategories_id GROUP BY products_id ORDER BY products_cardinality ASC", GetSQLValueString($colname_rs_products, "int"));
} else {
	//$query_rs_products = sprintf("SELECT * FROM v_products_list WHERE categories_id = %s", GetSQLValueString($colname_rs_products, "int"));
	$query_rs_products = sprintf("SELECT * FROM v_products_list WHERE categories_id = %s GROUP BY products_id ORDER BY products_cardinality ASC", GetSQLValueString($colname_rs_products, "int"));
}
...
if($_POST["subategorygroups"]){
	$specificQuery = "SELECT * FROM v_products_list WHERE categories_id = '$categoriesid' AND (";
	echo "subategorygroup count".count($_POST["subategorygroups"])."<br>";
	echo " ";
	
	foreach ($_POST["subategorygroups"] as $key) {
		if($thisCount < count($_POST["subategorygroups"])) {
			$specificQuery = $specificQuery." subcategories_subcategorygroup_id = '$key' OR ";
			echo "thisCount ".$thisCount."<br>";
			$thisCount++;
		} else {
			echo "thisCount ".$thisCount;
			$specificQuery = $specificQuery." subcategories_subcategorygroup_id = '$key') ";
		}
	}
	foreach ($_POST["subcategories"] as $key) {
		foreach ($key as $value) {
		}
	}
}
echo "<br>".$specificQuery;
*/
?>
<div id="products_for_category_main" style="position: relative; margin-top: -8px;">
	<!--<div class="manufacturer_button" id="manufacturer_button" style="margin-left: 10px; margin-top: 10px;">MANUFACTURER</div>-->
	<script>
		//$("#manufacturer_button").click(function(){
			//$(".product_box[boxManufacturerID!=1]").hide();
		//});
	</script>
	...
	<!-- LOOP -->
		...
		<?php do { ?>
			<div class="product_box" boxManufacturerID="<?php echo $row_rs_products['manufacturers_id']; ?>" boxProductsID="<?php echo $row_rs_products['products_id']; ?>" boxSubCatID="<?php echo $row_rs_products['subcategories_id']; ?>" boxSubCatGroupID="<?php echo $row_rs_products['subcategories_subcategorygroup_id']; ?>" style="position: relative; float: left; width: 197px; margin: 10px; min-height: 400px; 
			...
			">
				...
				<div style="position: relative; width:100%;">
					...
					<script>
						...
						?>
							$(".product_box[boxManufacturerID!=<? echo $_POST['manufacturers_id']?>]").hide();
						<?
						}
						?>
