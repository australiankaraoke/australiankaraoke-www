...
if (isset($_POST['category_id'])) {
  $colname_rs_products = $_POST['category_id'];
}

$productQuery = $_POST["productQuery"];
mysql_select_db($database_karaoke_db, $karaoke_db);

	//$query_rs_products = sprintf("SELECT * FROM v_products_list WHERE categories_id = %s", GetSQLValueString($colname_rs_products, "int"));
	$query_rs_products = "SELECT * FROM v_products_list WHERE (categories_title LIKE '%$productQuery%' OR products_title LIKE '%$productQuery%' OR products_sku LIKE '%$productQuery%' OR products_title LIKE '%$productQuery%' OR products_mini_description LIKE '%$productQuery%' OR products_description LIKE '%$productQuery%' OR products_features LIKE '%$productQuery%' OR subcategories_title LIKE '%$productQuery%' OR manufacturers_title LIKE '%$productQuery%') group by products_id";
...
//$query_rs_products = "SELECT * FROM v_products_list WHERE categories_id = '3' AND ((subcategories_subcategorygroup_id = '3' AND subcategories_id='5') OR (subcategories_subcategorygroup_id = '4' AND subcategories_id='11') OR subcategories_subcategorygroup_id<>'3' OR subcategories_subcategorygroup_id<>'4')";
...
$(".holder").jPages({
    ...
    perPage : 8,
    callback : function(){
	    $(".fittext_manuSku").fitText(1.1);
	    $(".fittext_prodTitle").fitText(1.5);
    }
});
</script>
</head>
...
<div id="products_for_category_main" style="position: relative; margin-top: -8px; margin-left: 10px;">
	<!-- LOOP -->
	<div class="holder" style="margin: 0 auto; text-align: center;"></div>
		<ul id="itemContainer">
		<?php do { ?>
		<li>
			<div class="product_box" boxProductsID="<?php echo $row_rs_products['products_id']; ?>" boxSubCatID="<?php echo $row_rs_products['subcategories_id']; ?>" boxSubCatGroupID="<?php echo $row_rs_products['subcategories_subcategorygroup_id']; ?>" style="position: relative; float: left; width: 197px; margin: 15px; min-height: 400px; 
			...
			">
				...
				<div style="position: relative; width:100%;">
					...
					?>
						...
						<!--<a href="/products/index.php?product_id=<?php echo $row_rs_products['products_id']; ?>&category=<?php echo $row_rs_products['categories_title']; ?>&subcategory=<?php echo $row_rs_products['subcategories_title']; ?>&manufacturer=<?php echo $row_rs_products['manufacturers_title']; ?>&product=<?php echo $row_rs_products['products_sku']; ?>" class="details_button" style="box-shadow: 0px 0px 1px 2px rgba(0,0,0,0.2); float: left; margin: 10px 0px 7px 10px;">DETAILS</a>-->
		            ...
		            <div>
			            <h1 style="color: #891a9c; padding: 5px 10px 5px 10px; height: 35px; padding-top: 10px; background-color: rgba(0,0,0,0.1); width: 90%; text-align: center;" id="fittext_manuSku_id_<?php echo $row_rs_products['products_id']; ?>" class="fittext_manuSku"><span style=""><?php echo $row_rs_products['manufacturers_title']; ?> <?php echo $row_rs_products['products_sku']; ?></span></h1>
						
						<h2 style="color: #333; padding: 5px 10px 5px 10px; height: 25px; font-size: 16px; width: 90%; text-align: center;" id="fittext_prodTitle_id_<?php echo $row_rs_products['products_id']; ?>" class="fittext_prodTitle"><?php echo $row_rs_products['products_title']; ?></h2>
						<script>
					...
					<div class="product_description" data-len="100" style="padding: 5px 10px 5px 10px; font-size: 13px; color: #666666; height: 75%; display: block;" id="fittext_prodDescr_id_<?php echo $row_rs_products['products_id']; ?>" class="fittext_prodDescr"><?php echo $row_rs_products['products_mini_description']; ?></div>
